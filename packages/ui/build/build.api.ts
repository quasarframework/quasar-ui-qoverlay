...
async function generateApiJson(): Promise<ComponentApi> {
  ...
  return JSON.parse(fs.readFileSync(path.join(apiDir, 'QOverlay.json'), 'utf-8')) as ComponentApi
}
...
${typeImports}
export interface QOverlay extends ComponentPublicInstance {
${[props, methods].filter(Boolean).join('\n')}
}

export interface QOverlayProps {
${props}
}

export const version: string

export const QOverlay: ComponentOptions

export interface QOverlayPlugin {
  version: string
  QOverlay: ComponentOptions
  install(app: Application): void
}

declare const plugin: QOverlayPlugin
export default plugin
export * from './types'
export as namespace QOverlay
`
}
